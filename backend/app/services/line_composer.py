...
SITUATION_GUIDES = {
    ...
    "cancel_failed": (
        "キャンセル処理が完了しなかったことを詫び、再試行か担当者対応を案内する。"
        "システム・エラー・障害など内部の事情には一切触れず、原因も説明しない"
        "（患者に必要なのは次にどうすればよいかだけ）。"
    ),
...
# 症状を勝手に推測する文（「お痛みは大丈夫ですか」等）を止めるための語。
# 「お大事に」は症状の詮索ではなく受付の結びの挨拶なので対象外にする。
# 含めていたため「キャンセルを承りました。お大事になさってください。」のような
# 自然な文まで棄却され、定型文へ落ちていた。
_SYMPTOM_REPLY_WORDS = (
    "痛み", "痛く", "痛い", "症状", "患部", "お加減", "体調", "お怪我", "怪我",
)
