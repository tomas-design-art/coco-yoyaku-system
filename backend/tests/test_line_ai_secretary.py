...
# ── 2026-08-26 本番の「AI返信に失敗」再発防止 ──
# 実機でキャンセルのたびに必ず2件セットで発生していた:
#   situation=cancel_done   reason=contradiction
#   situation=cancel_failed reason=unsupported_claim


def test_cancel_done_needs_slot_facts_or_valid_dates_get_rejected():
    """取消完了は厳格検算の対象。事実を渡さないと正しい返信まで棄却される。"""
    from app.services.line_composer import _has_temporal_contradiction

    with_facts = {"date": "2026/08/27", "start": "14:00", "end": "15:00", "patient_message": "はい"}
    assert _has_temporal_contradiction(
        with_facts, "8/27(木) 14:00のご予約をキャンセルいたしました。", "cancel_done"
    ) is False
    # 捏造した日時は今も止める
    assert _has_temporal_contradiction(
        with_facts, "9/5(金) 19:00のご予約をキャンセルしました。", "cancel_done"
    ) is True
    # 事実ゼロだと日付を書いた時点で棄却される（呼び出し側が渡す必要がある証拠）
    assert _has_temporal_contradiction(
        {"patient_message": "はい"}, "8/27のご予約をキャンセルしました。", "cancel_done"
    ) is True


def test_closing_courtesy_is_not_symptom_speculation():
    """「お大事に」は結びの挨拶。症状の詮索として棄却しない。"""
    from app.services.line_composer import _has_unsupported_claim

    context = {"date": "2026/08/27", "start": "14:00", "patient_message": "はい"}
    assert _has_unsupported_claim(context, "キャンセルを承りました。お大事になさってください。") is False
    # 症状を勝手に推測する文は今も止める
    assert _has_unsupported_claim(context, "お痛みは大丈夫でしょうか。") is True


def test_cancel_failed_guide_forbids_internal_causes():
    """取消失敗の案内で内部事情に触れさせない（触れるとガードに棄却されテンプレへ落ちる）。"""
    from app.services.line_composer import SITUATION_GUIDES, _has_unsupported_claim

    guide = SITUATION_GUIDES["cancel_failed"]
    assert "システム" in guide and "触れず" in guide

    context = {"patient_message": "はい", "reason": "既にキャンセル済み"}
    natural = "申し訳ございません。キャンセル手続きが完了できませんでした。もう一度お試しいただくか担当者へご連絡ください。"
    assert _has_unsupported_claim(context, natural) is False
    assert _has_unsupported_claim(context, "システム障害のため対応できません。") is True


# ── 2026-08-26 深夜の実機テストで見つかった2件 ──


def test_cancel_done_must_not_read_as_booking_accepted():
    """取消完了で「ご予約、承りました」と送らない（本番で実際に送られた事故）。"""
    from app.services.line_composer import _has_wrong_booking_outcome

    sent_in_production = "8月31日16:15からのご予約、承りました。また改めてご希望の日時がございましたら、いつでもご連絡くださいね。"
    assert _has_wrong_booking_outcome("cancel_done", sent_in_production) is True

    # 正しい取消完了文は通る
    for reply in [
        "8月31日16:15のご予約をキャンセルいたしました。",
        "8/31(月) 16:15のご予約をキャンセルしました。またのご来院をお待ちしております。",
        "キャンセルを承りました。お大事になさってください。",
    ]:
        assert _has_wrong_booking_outcome("cancel_done", reply) is False

    # 取消に一言も触れない返信も止める（完了が伝わらない）
    assert _has_wrong_booking_outcome("cancel_done", "承知いたしました。またご連絡くださいね。") is True

    # 他situationの既存挙動は変えない
    assert _has_wrong_booking_outcome("cancel_aborted", "キャンセルしました。") is True
    assert _has_wrong_booking_outcome("cancel_failed", "ご予約を承りました。") is True
    assert _has_wrong_booking_outcome("confirmed", "ご予約を承りました。") is False


def test_time_only_reply_keeps_the_date_under_discussion():
    """候補提示後に時刻だけ言われても、話している日を今日へ飛ばさない。"""
    from app.api.line import _mentions_explicit_date

    # 日を指定していない＝提示済みの日を維持すべき発言
    for text in ["17時ならどお？", "18時くらいに時田先生空いてない？", "夕方以降ありませんか？", "もっと遅い時間は？"]:
        assert _mentions_explicit_date(text) is False, text

    # 日を明言している＝その日へ移すべき発言
    for text in ["明日の17時は？", "来週の月曜は？", "8/31の夕方は？", "9月1日でお願い", "今日は空いてる？", "土曜ならどう？"]:
        assert _mentions_explicit_date(text) is True, text
