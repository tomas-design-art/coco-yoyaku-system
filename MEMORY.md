...
## LINE予約成功後の逆結果返信防止 (2026-09-04)

- 実機で、候補番号の選択により予約ボードへ予約が作成された一方、患者には「予約が埋まっている」と逆の結果が返信されたことを確認。
- `confirmed` / `change_done` / `cancel_done` はLLMへ自由作文させず、DB操作結果と確定日時から固定返信を生成する。
- 年付き確定日と返信中の曜日を照合し、日付付き・日付省略のどちらでも誤曜日を矛盾として拒否する。
- LINE予約の`source_ref`へ`webhookEventId`を保存し、部分一意indexで同一イベントの二重予約を防止。再試行時は既存予約を返して会話状態更新と返信を再開する。
- 永続キュー経路は本文＋8秒のプロセス内dedupを使わず、`webhookEventId`を正本にする。legacy同期経路だけ本文dedupを維持する。
- 取消完了返信には、実際に取消した予約の日付・開始時刻を必ず含める。
- 回帰テストはLINE＋Alembic 156件成功。backend全体は既知4失敗のみで419件成功。migration 028を実PostgreSQLへ適用しindex実在も確認。
